            ...
            <div class="container-fluid">
                ...
                <div class="row">
                                    ...
                                    <div class="card component-card_4" style="width:100%;">
                                        <div class="card-body align-items-center">
                                            ...
                                            @php
                                                ...
                                                if(!empty($teamInfo->image) && \Storage::disk(config('filesystems.default'))->exists(config('constants.teams.image_path').$teamInfo->image))
                                                {
                                                    $imagePath = (get_image_url(config('constants.teams.image_path'), $teamInfo->image) ?? '');
                                                }
                                            @endphp
                                            ...
                                            <div class="user-info pb-0">
                                                <h5 class="card-user_name">{{ ucwords($teamInfo->name) }}</h5>
                                                <p class="card-user_occupation">Players : {{ $teamInfo->auction_players_count }}/{{ $auction->maximum_player }}</p>
                                                <div class="card-star_rating">
                                                   <span class="progress-bar bg-gradient-primary">Total Points : {{ $auction->point_per_team }}</span>
                                                </div>
                                                <div class="card-star_rating">
                                                   <span class="progress-bar bg-gradient-warning">Used Points : {{ $teamInfo->total_points }}</span>
                                                </div>
                                                <div class="card-star_rating">
                                                   <span class="progress-bar bg-gradient-danger">Remaining Points : {{ $auction->point_per_team - $teamInfo->total_points }}</span>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                    ...
                    <div class="col-lg-8 layout-spacing">
                        <div class="row">
                            @foreach($teamInfo['auctionPlayers'] as $auction_players)
                                ...
                                <div class="col-sm-4">
                                    <div class="user-profile layout-spacing">
                                        <div class="widget-content widget-content-area">
                                            ...
                                            @php
                                                $imagePath = asset('admin-assets/images/player.png');
                                                if(!empty($auction_players['player']->image) && \Storage::disk(config('filesystems.default'))->exists(config('constants.players.image_path').$auction_players['player']->image))
                                                {
                                                    $imagePath = (get_image_url(config('constants.players.image_path'), $auction_players['player']->image) ?? '');
                                                }
                                            @endphp

                                            <div class="text-center user-info mt-3">
                                                <img src="{{ $imagePath }}" width="100px" alt="avatar">
                                                <p class="">{{ ucwords($auction_players['player']->name) }}</p>
                                            </div>

                                            @if($auction_players['status'] == 1)
                                                <div class="text-center mt-0">
                                                    <p><b>Sold Points : </b>{{ $auction_players['points'] }} Points</p>
                                                </div>
                                            @endif
                                            ...
                                            <div class="user-info-list">
                                                <ul class="contacts-block list-unstyled m-0 text-center" style="max-width:100%">
                                                    ...
                                                    @if($auction_players['player']->age)
                                                        <li class="contacts-block__item">
                                                            <b>Age : </b> {{ $auction_players['player']->age }}
                                                        </li>
                                                    @endif

                                                    @if($auction_players['player']->category_id)
                                                        <li class="contacts-block__item">
                                                            <b>Category : </b> {{ ucwords($auction_players['player']['categories']->name) }}
                                                        </li>
                                                    @endif

                                                    @if($auction_players['player']->tag)
                                                        <li class="contacts-block__item">
                                                            <b>Tag : </b> 
                                                            @if($auction_players['player']->tag == 1)
                                                                Owner
                                                            @elseif($auction_players['player']->tag == 2)
                                                                Co-Owner
                                                            @elseif($auction_players['player']->tag == 3)
                                                                Captain
                                                            @elseif($auction_players['player']->tag == 4)
                                                                Vice Captain
                                                            @elseif($auction_players['player']->tag == 5)
                                                                Icon
                                                            @elseif($auction_players['player']->tag == 6)
                                                                Retain
                                                            @endif
                                                        </li>
                                                    @endif

                                                    @if($auction_players['player']->playing_style)
                                                        <li class="contacts-block__item">
                                                            <b>Playing Style : </b> {{ $auction_players['player']->playing_style }}
                                                        </li>
                                                    @endif

                                                    @if($auction_players['player']->specification_1)
                                                        <li class="contacts-block__item">
                                                            <b>Specification 1 : </b> {{ $auction_players['player']->specification_1 }}
                                                        </li>
                                                    @endif

                                                    @if($auction_players['player']->specification_2)
                                                        <li class="contacts-block__item">
                                                            <b>Specification 2 : </b> {{ $auction_players['player']->specification_2 }}
                                                        </li>
                                                    @endif

                                                    @if($auction_players['player']->specification_3)
                                                        <li class="contacts-block__item">
                                                            <b>Specification 3 : </b> {{ $auction_players['player']->specification_3 }}
                                                        </li>
                                                    @endif
                                                </ul>                              
                                            </div>
