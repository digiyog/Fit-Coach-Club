var PostReport = (function() {
    ...
    return {
         ...
         */
        init: function() {
            PostReport.getPostReport();
            PostReport.changeStatus();
            PostReport.destroyRecord();
            PostReport.destroyPostRecord();
            PostReport.dataTableCustomFilter();
            PostReport.initializeComponents();
        ...
        getPostReport: function() {
            ...
            data_table = table = $dataTable.DataTable({
                initComplete: function() {
                    ...
                    $('.btn-toolbar').append(
                        '<button type="button" title="Delete Post" class="btn btn-icon btn-rounded btn-primary btn-outline dt-delete-post" disabled> <i class="fa fa-trash" aria-hidden="true"></i> &nbsp; Delete Post </button> '
                    );
                ...
                ajax: {
                    ...
                    data: function(d) {
                        d.name = $("select[name=name_filter]").val();
            ...
            $dataTable
                ...
                .on("click", 'input[type="checkbox"]', function(e) {
                    ...
                    if (
                        ...
                        $(".dt-delete-post").prop("disabled", false);
                    } else {
                        $(".change-status").prop("disabled", true);
                        $(".dt-delete").prop("disabled", true);
                        $(".dt-delete-post").prop("disabled", true);
                    }
                    ...
                    PostReport.updateDataTableSelectAllCtrl(table);
         ...
         * Destroy record.
         */
        destroyPostRecord: function() {
            var $data_table_container = $(".data-table-container");
            var $dataTable = $(".dataTable");

            // Handle form submission event
            $data_table_container.on("click", ".dt-delete-post", function() {
                // Iterate over all selected checkboxes
                var ids = [];
                $.each(rows_selected, function(index, rowId) {
                    ids.push(rowId.id);
                    ...
                    buttons: [
                        [
                            ...
                            function(instance, toast) {
                                ...
                                $.ajax({
                                    type: "DELETE",
                                    url: $dataTable.data("destroy-post-url"),
                                    data: { ids: ids },
                                    beforeSend: function() {
                                        $(".dt-delete").prop("disabled", true);
                                        $(".dt-delete-post").prop("disabled", true);
